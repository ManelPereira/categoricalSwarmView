class BaseCategory:
    def __init__(self, name, probability,position,canvas,color):
        ...
        self.item = self.canvas.create_text(self.x, self.y, anchor="n",fill=self.color,text= f"{self.name} ({int(self.sim_category*100)} %) ({int(self.x),int(self.y)})",font=("Purisa", 14))
    ...
    def _set_sim_category(self, probability):
        self._sim_category = probability
        self.canvas.itemconfig(self.item, text= f"{self.name} ({int(self.sim_category*100)} %) ({int(self.x),int(self.y)})")
    @classmethod
    def colors(cls):
        #generate dark colors
        return ["navy","teal","aqua","red","green","blue","orange","purple","pink","brown","black","white","grey","cyan","magenta","gold","torquoise","olive","maroon"]
